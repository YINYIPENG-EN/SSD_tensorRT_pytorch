...
from utils.box_utils import ssd_correct_boxes
...
class SSD(object):
    ...
    def generate(self):
        ...
        if self.onnx or self.engine:
            self.net = DetectMultiBackend(self.model_path, device, False)
            self.detect = Detect(self.num_classes, 0, 200, self.confidence, self.nms_iou)
        else:
    ...
    def detect_image(self, image):
        ...
        with torch.no_grad():
            ...
            if self.onnx or self.engine:  # get onnx or engine outputs
                loc = preds[0]
                conf = preds[1]
                priors = preds[2]
            ...
            for i in range(preds.size(1)):
                j = 0
                while preds[0, i, j, 0] >= self.confidence:  # preds[...,0] 第0个维度是conf
                    #---------------------------------------------------#
                    #   score为当前预测框的得分
                    #   label_name为预测框的种类
                    #---------------------------------------------------#
